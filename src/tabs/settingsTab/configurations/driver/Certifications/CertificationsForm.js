...
const CertificationsForm = ({
    ...
    onSubmit: async (values) => {
      try {
        if (isEditMode && editingId) {
          await putRequest(`/api/certifications/${editingId}`, values);
          showSuccess("Certification updated successfully");
        } else {
          await postRequest("/api/certifications", values);
  ...
  const handleDelete = async () => {
    try {
      await deleteRequest(`/api/certifications/${editingId}`);
      showSuccess("Certifications deleted successfully");
      onSuccess();
    } catch (error) {
      showError("Failed to delete Certifications");
