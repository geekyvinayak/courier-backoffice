...
import { Checkbox, FormControl, InputLabel, MenuItem, Select, FormControlLabel, Button, Box, Typography } from '@mui/material';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const selectedVehicleID = vehiclesResponse.find(vehicle => vehicle.displayId == vehicleResponse.vehicleId  );
     
        const equivalenciesResponse = await getRequest(`/vehicleEquivalency/availableVehicleId/${selectedVehicleID.id}`);
                ...
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      name="equivalencies"
                      value={equivalency}
                      checked={values?.equivalencies?.includes(equivalency.toString())}
                      onChange={(e) => {
